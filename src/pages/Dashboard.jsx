...
const summaryTableProps = {
  ...
  rowClassName: (_, i) =>
    i % 2 === 0
      ...
      : "bg-slate-50/20 border-b !border-slate-100",
};
...
const parseSummaries = (payload) => {
  ...
  if (
    payload.grouped?.order_date &&
    Array.isArray(payload.grouped.order_date)
  ) {
    ...
    payload.summaries?.date_wise &&
    Array.isArray(payload.summaries.date_wise)
  ) {
    ...
    if (Array.isArray(dw)) {
      orderDateSummaryRows = dw.map((item) => ({
        ...
        cost: (
          Number(item.cost_amt) ||
          Number(item.total_cost_amount) ||
          0
        ).toFixed(2),
      }));
    } else {
  ...
  let tempStatusRows = [];
  if (
    payload.grouped?.order_summary &&
    Array.isArray(payload.grouped.order_summary)
  ) {
    ...
    payload.summaries?.status_wise &&
    Array.isArray(payload.summaries.status_wise)
  ) {
    ...
    payload.summary?.status_wise_counts &&
    Array.isArray(payload.summary.status_wise_counts)
  ) {
  ...
  if (payload.grouped?.courier && Array.isArray(payload.grouped.courier)) {
    ...
    payload.summaries?.courier_wise &&
    Array.isArray(payload.summaries.courier_wise)
  ) {
...
export default function Dashboard() {
  ...
  const [ordersPaymentStatusDraft, setOrdersPaymentStatusDraft] =
    useState("all");
  ...
  const [paymentsOrderFilterDraft, setPaymentsOrderFilterDraft] =
    useState("All");
  ...
  const [paymentsSummaries, setPaymentsSummaries] = useState({
    orderDateSummaryRows: [],
    statusSummaryRows: [],
    courierSummaryRows: [],
  });
  const [returnsSummaries, setReturnsSummaries] = useState({
    orderDateSummaryRows: [],
    statusSummaryRows: [],
    courierSummaryRows: [],
  });
  ...
  const [ordersFilter, setOrdersFilter] = useState("all"); // 'all', 'delivered', 'cancelled', 'pending', 'sla', 'other'
  ...
  useEffect(() => {
    localStorage.setItem(
      "crm_dashboard_sidebar_open",
      JSON.stringify(dashboardSidebarOpen),
    );
  ...
  const fetchPendingPayments = useCallback(async () => {
    ...
    try {
      ...
      setPendingTotal(
        Number(data.total_count ?? data.total_rows ?? data.total) ||
          (data.data || []).length,
      );
  ...
  const fetchOrdersAnalysis = useCallback(async () => {
    ...
    try {
      ...
      if (ordersFilter === "delivered") {
        ...
        extraFilter = { order_status: ["SHIPPED"] };
        ...
        ordersFilter === "approved-claims" ||
        ordersFilter === "pending-claims"
      ) {
      ...
      const payload = {
        filter_data: {
          ...
          ...extraFilter,
          ...(ordersOrderId.trim()
            ? { platform_order_id: ordersOrderId.trim() }
            : {}),
    ...
    try {
      ...
      const payload = {
        filter_data: {
          start_date: selectedDateRange.from || "",
          end_date: selectedDateRange.to || "",
          ...(paymentsOrderId.trim()
            ? { order_id: paymentsOrderId.trim() }
            : {}),
  ...
  const fetchReturnsAnalysis = useCallback(async () => {
    ...
    try {
      ...
      if (returnsFilter === "transit") {
        endpoint = "/get-return-in-transit";
  ...
  const exportPendingPayments = async () => {
    ...
    try {
      ...
      link.setAttribute(
        "download",
        `pending_payments_${activeAccount.account_name || "account"}.csv`,
      );
      ...
      try {
        ...
        if (!activeAccount) {
          ...
          const matched = savedId
            ? list.find((acc) => String(acc.id) === savedId)
            : null;
  ...
  const requiresAccountSelection =
    !loading && !loadingAccounts && accounts.length > 0 && !activeAccount;
  ...
  const slaBreachedOrdersVal =
    dashboardCards.unsettled_pickup?.total_orders || 0;
  ...
  const returnsChartData = [
    {
      name: "On The Way",
      count: Number(dashboardCards.return_in_transit?.total_orders || 0),
      color: "#f59e0b",
    },
    {
      name: "Received",
      count: Number(dashboardCards.return_received?.total_orders || 0),
      color: "#10b981",
    },
    {
      name: "Not Received",
      count: Number(dashboardCards.return_not_received?.total_orders || 0),
      color: "#f43f5e",
    },
    {
      name: "Wrong/Damaged",
      count: Number(dashboardCards.return_mismatch?.total_orders || 0),
      color: "#ef4444",
    },
  ];
        ...
        <div className="space-y-4 max-w-[1700px] mx-auto">
          ...
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
            ...
            <div className="flex flex-col gap-0.5">
              <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none border-l-2 border-primary pl-1.5">
            ...
            >
              ...
              {activeTab === "action-required" && (
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div className="bg-white rounded-2xl border border-slate-100 p-5 shadow-soft hover:shadow-card hover:-translate-y-0.5 transition-all flex justify-between items-center group">
                      ...
                      <div className="flex flex-col items-end gap-2 shrink-0">
                        <span className="text-xs font-black text-slate-800 group-hover:scale-105 transition-transform">
                          {formatCount(pendingTotal)}
                        </span>
                        <Sparkline
                          color="#E24A31"
                          points="M 5 25 Q 25 10 50 15 T 95 5"
                        />
                      </div>
                    </div>
                  ...
                  <div
                    className={`grid gap-3 xl:gap-3 ${dashboardSidebarOpen ? "xl:grid-cols-[320px_minmax(0,1fr)]" : "grid-cols-1"}`}
                  >
                    {dashboardSidebarOpen && (
                        ...
                        <SummaryTable
                          ...
                          cols={[
                            ...
                            },
                            {
                              ...
                              right: true,
                              color: () =>
                                "text-primary text-right font-extrabold",
                            },
                            {
                              key: "cost",
                              label: "Cost Amt",
                              right: true,
                              color: () =>
                                "text-emerald-600 text-right font-bold",
                            },
                          ]}
                        />
                        <SummaryTable
                    ...
                    <div className="bg-white rounded-2xl border border-slate-100 p-5 shadow-soft min-w-0">
                      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3 mb-5 border-b border-slate-50 pb-4">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="bg-blue-50 text-blue-600 text-xs font-extrabold px-2.5 py-1 rounded-full">
                            {formatCount(pendingTotal)}
                          </span>
                        </div>
                      ...
                      <div className="crm-panel-muted p-4 sm:p-5 mb-5 shadow-soft">
                        <div className="flex flex-wrap items-end gap-3">
                          ...
                          <div className="flex flex-col gap-1.5 min-w-[200px] flex-1">
                            <label className="ml-1 text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none">
                              SKU
                            </label>
                            <div className="relative group">
                              ...
                              <input
                                ...
                                onChange={(e) =>
                                  setPendingSkuDraft(e.target.value)
                                }
                          ...
                          <div className="flex flex-col gap-1.5 min-w-[200px] flex-1">
                            ...
                            <div className="relative group">
                              ...
                              <input
                                ...
                                onChange={(e) =>
                                  setPendingOrderIdDraft(e.target.value)
                                }
                          ...
                          <div className="flex flex-col gap-1.5 w-44">
                            ...
                            <div className="relative">
                              <select
                                ...
                                onChange={(e) =>
                                  setPendingOrderFilterDraft(e.target.value)
                                }
                      ...
                      {pendingTotal > 0 && (
                        <div className="flex flex-col sm:flex-row items-center justify-between mt-4 pt-3 border-t border-slate-100 text-xs text-slate-400 font-bold gap-3">
                          <div className="flex items-center gap-3">
                            ...
                            <span className="text-[10px] opacity-60 font-bold ml-2">
                              Rows per page:
                            </span>
                            <select
                              value={pendingLimit}
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <span className="font-extrabold text-slate-700">
                              Page {pendingPage} of{" "}
                              {Math.ceil(pendingTotal / pendingLimit) || 1}
                            </span>
                            <button
                              disabled={
                                pendingPage >=
                    ...
                    {[
                      ...
                      >
                        <div className="space-y-3">
                          <div
                            className={`p-2.5 rounded-xl ${card.color.bg} ${card.color.text} w-fit`}
                          >
                            <card.icon size={16} />
                          </div>
                          <div>
                      ...
                      <div className="flex items-center gap-2 mb-4">
                        ...
                        <div>
                          ...
                          <p className="text-[10px] font-bold text-slate-400 mt-0.5">
                            Current stage of all received orders
                          </p>
                        </div>
                      </div>
                        ...
                        {[
                          {
                            ...
                            count: dashboardCards.all_orders?.total_orders || 0,
                            key: "all_orders",
                          ...
                          },
                          {
                            ...
                            count: dashboardCards.delivered?.total_orders || 0,
                            key: "delivered",
                            barColor: "bg-emerald-500",
                          },
                          {
                            filterKey: "shipped",
                            title: "On the Way (Shipped)",
                            count: dashboardCards.shipped?.total_orders || 0,
                            key: "shipped",
                          ...
                          },
                          {
                            ...
                            count:
                              dashboardCards.out_for_delivery?.total_orders ||
                              0,
                          ...
                          },
                          {
                            ...
                            title: "Ready to Ship",
                            count:
                              dashboardCards.ready_to_ship?.total_orders || 0,
                            key: "ready_to_ship",
                          ...
                          },
                          {
                            filterKey: "cancelled",
                            title: "Cancelled",
                            count: dashboardCards.cancelled?.total_orders || 0,
                            key: "cancelled",
                            barColor: "bg-slate-500",
                          },
                          {
                            filterKey: "returned",
                            title: "Total Returns (RTO & Customer)",
                            count:
                              dashboardCards.all_orders?.breakdown
                                ?.all_returns || 0,
                            key: "all_returns",
                          ...
                          },
                          {
                            ...
                            count:
                              dashboardCards.all_orders?.breakdown?.other || 0,
                            key: "other",
                            barColor: "bg-slate-400",
                          },
                        ].map((stage, idx) => {
                          const total = Number(
                            dashboardCards.all_orders?.total_orders || 1,
                          );
                          const percentage =
                            total > 0
                              ? Math.round((Number(stage.count) / total) * 100)
                              : 0;
                          ...
                          return (
                            <div
                              ...
                              onClick={() => setOrdersFilter(stage.filterKey)}
                            >
                              <div className="flex items-center justify-between text-xs">
                                <span className="font-bold text-slate-600 uppercase tracking-tight text-[10px]">
                                  {stage.title}
                                </span>
                              ...
                              <div className="h-2 w-full bg-slate-100 rounded-full overflow-hidden">
                                <div
                                  className={`h-full transition-all duration-1000 ease-out ${stage.barColor}`}
                    ...
                    <div className="bg-white rounded-2xl border border-slate-100 p-5 shadow-soft flex flex-col justify-between">
                      <div>
                        <div className="flex items-center gap-2 mb-4">
                          ...
                          <div>
                            ...
                            <p className="text-[10px] font-bold text-slate-400 mt-0.5">
                              Track courier disputes and reimbursement status
                            </p>
                          </div>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-3 mt-4">
                          {[
                            {
                              filterKey: "unsettled-pickup",
                              title: "Unsettled Pickups",
                              key: "unsettled_pickup",
                              bg: "bg-violet-50/50 text-violet-700 border-violet-100",
                            },
                            {
                              filterKey: "cancel-pickup",
                              title: "Cancelled Pickups",
                              key: "cancel_pickup",
                              bg: "bg-fuchsia-50/50 text-fuchsia-700 border-fuchsia-100",
                            },
                            {
                              filterKey: "approved-claims",
                              title: "Approved Claims",
                              key: "approved_claims",
                              bg: "bg-emerald-50/50 text-emerald-700 border-emerald-100",
                            },
                            {
                              filterKey: "pending-claims",
                              title: "Pending Claims",
                              key: "pending_claims",
                              bg: "bg-amber-50/50 text-amber-700 border-amber-100",
                            },
                          ].map((item, idx) => {
                              ...
                              >
                                ...
                                <div className="mt-3 flex items-baseline justify-between">
                                  <h5 className="text-lg font-black">
                                    {formatCount(dataObj.total_orders)}
                                  </h5>
                                  <span className="text-[10px] font-bold opacity-80">
                                    ₹{formatCount(dataObj.total_cost || 0)}
                                  </span>
                                </div>
                              </div>
                            );
                      ...
                      <div
                        className={`mt-5 p-3 rounded-xl border flex items-center justify-between text-xs cursor-pointer hover:bg-slate-100 transition-all ${
                          ordersFilter === "wrong-return-claims"
                      ...
                      >
                        <span className="font-extrabold text-slate-600">
                          Wrong Returns Claims Filed
                        </span>
                        <h6 className="font-black text-red-600">
                          {formatCount(
                            dashboardCards.wrong_return_claims?.total_orders,
                          )}{" "}
                      ...
                      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3 mb-5 border-b border-slate-50 pb-4">
                        <div className="flex items-center gap-2">
                          <h3 className="text-lg font-black text-slate-800 uppercase tracking-tight">
                            ...
                            {ordersFilter === "ready_to_ship" &&
                              "Ready to Ship Orders"}
                            {ordersFilter === "shipped" && "Shipped Orders"}
                            {ordersFilter === "out_for_delivery" &&
                              "Out for Delivery Orders"}
                            {ordersFilter === "returned" && "Returned Orders"}
                            {ordersFilter === "pending" &&
                              "Pending Payment Orders"}
                            {ordersFilter === "sla" &&
                              "Unsettled Pickup Orders"}
                            {ordersFilter === "unsettled-pickup" &&
                              "Unsettled Pickup Orders"}
                            {ordersFilter === "cancel-pickup" &&
                              "Cancelled Pickup Orders"}
                            {ordersFilter === "approved-claims" &&
                              "Approved Claims Records"}
                            {ordersFilter === "pending-claims" &&
                              "Pending Claims Records"}
                            {ordersFilter === "wrong-return-claims" &&
                              "Wrong Return Claims Records"}
                            {ordersFilter === "other" &&
                              "Other / Uncategorized Orders"}
                          </h3>
                          <span className="bg-indigo-50 text-indigo-600 text-xs font-extrabold px-2.5 py-1 rounded-full">
                          ...
                          <div className="flex flex-col gap-1.5 min-w-[200px] flex-1">
                            ...
                            <div className="relative group">
                              ...
                              <input
                                ...
                                onChange={(e) =>
                                  setOrdersSkuDraft(e.target.value)
                                }
                                placeholder="Search SKU..."
                                className="h-9 w-full bg-slate-50 border border-slate-200 rounded-xl pl-10 pr-4 text-xs font-bold text-slate-700 outline-none focus:bg-white focus:border-primary transition-all shadow-inner"
                              />
                          ...
                          <div className="flex flex-col gap-1.5 min-w-[200px] flex-1">
                            ...
                            <div className="relative group">
                              ...
                              <input
                                ...
                                onChange={(e) =>
                                  setOrdersOrderIdDraft(e.target.value)
                                }
                                placeholder="Search Order ID..."
                                className="h-9 w-full bg-slate-50 border border-slate-200 rounded-xl pl-10 pr-4 text-xs font-bold text-slate-700 outline-none focus:bg-white focus:border-primary transition-all shadow-inner"
                              />
                          ...
                          <div className="flex flex-col gap-1.5 w-44">
                            ...
                            <div className="relative">
                              <select
                                ...
                                onChange={(e) =>
                                  setOrdersStatusDraft(e.target.value)
                                }
                                className="h-9 w-full appearance-none rounded-xl border border-slate-200 bg-slate-50 px-4 text-xs font-bold text-slate-700 outline-none focus:bg-white focus:border-primary transition-all shadow-inner hover:border-slate-300"
                              >
                                ...
                                <option value="DOOR_STEP_EXCHANGED">
                                  DOOR_STEP_EXCHANGED
                                </option>
                          ...
                          <div className="flex flex-col gap-1.5 w-44">
                            ...
                            <div className="relative">
                              <select
                                ...
                                onChange={(e) =>
                                  setOrdersPaymentStatusDraft(e.target.value)
                                }
                                className="h-9 w-full appearance-none rounded-xl border border-slate-200 bg-slate-50 px-4 text-xs font-bold text-slate-700 outline-none focus:bg-white focus:border-primary transition-all shadow-inner hover:border-slate-300"
                              >
                      ...
                      {ordersTotal > 0 && (
                        <div className="flex items-center justify-between mt-4 pt-3 border-t border-slate-100 text-xs text-slate-400 font-bold">
                          <div>
                            Showing{" "}
                            {Math.min(
                              (ordersPage - 1) * ordersLimit + 1,
                              ordersTotal,
                            )}{" "}
                            - {Math.min(ordersPage * ordersLimit, ordersTotal)}{" "}
                            of {ordersTotal}
                          </div>
                          <div className="flex items-center gap-2">
                            <button
                            ...
                            <span className="font-extrabold text-slate-700">
                              Page {ordersPage} of{" "}
                              {Math.ceil(ordersTotal / ordersLimit) || 1}
                            </span>
                            <button
                              disabled={
                                ordersPage >=
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 items-stretch">
                    ...
                    <div className="bg-white rounded-2xl border border-slate-100 p-5 shadow-soft flex flex-col justify-between">
                      <div>
                        ...
                        <div className="space-y-2 mt-4">
                          {[
                            ...
                            {
                              filterKey: "mismatch",
                              key: "payment_mismatch",
                              title: "Wrong/Unmatched Payments",
                              color: "text-rose-600",
                              icon: FiAlertCircle,
                              bg: "bg-rose-50",
                            },
                            ...
                            return (
                              ...
                              >
                                <div className="flex items-center gap-2.5">
                                  <div
                                    className={`p-1.5 rounded-lg ${p.bg} ${p.color}`}
                                  >
                                    <p.icon size={14} />
                                  </div>
                                  <div>
                                    ...
                                    <div className="mt-1 text-xs font-black text-slate-800">
                                      {formatCount(live.total_orders)} orders
                                    </div>
                                  </div>
                                </div>
                                <div
                                  className={`text-sm font-black ${p.color}`}
                                >
                                  ₹
                                  {formatCount(
                                    p.key === "pending_payment"
                                      ? live.total_pending_amount
                                      : live.total_cost,
                                  )}
                                </div>
                              </div>
                      ...
                      <div className="mt-4 pt-3 border-t border-slate-100 flex items-center justify-between text-xs">
                        ...
                        <span className="font-black text-emerald-600 font-bold select-all">
                          ₹{formatCount(headlineMetrics.received_bank_amount)}
                        </span>
                      </div>
                    </div>
                          ...
                          const chartData = [
                            {
                              ...
                              value: Math.max(
                                0,
                                Number(accountStatus.net_profit || 0),
                              ),
                            ...
                            },
                            {
                              name: "Ad Spend",
                              value: Number(
                                headlineMetrics.advertisement_cost || 0,
                              ),
                            ...
                            },
                            {
                              name: "Return Loss",
                              value: Number(
                                accountStatus.return_not_received_loss || 0,
                              ),
                              color: "#ef4444", // Red-500
                            },
                            {
                              name: "Payment Loss",
                              value: Number(accountStatus.payment_loss || 0),
                              color: "#6366f1", // Indigo-500 - distinctly different from Red!
                            },
                          ].filter((d) => d.value > 0);

                          if (chartData.length === 0) {
                            return (
                              <div className="text-center text-slate-400 py-6 text-xs font-bold">
                            ...
                            <ResponsiveContainer width="100%" height="100%">
                              <PieChart>
                                <Pie
                                  ...
                                  outerRadius={70}
                                  paddingAngle={5}
                                  minAngle={15} // Ensure tiny slices are easily visible and hoverable
                                  dataKey="value"
                                >
                    ...
                    <div className="bg-white rounded-2xl border border-slate-100 p-5 shadow-soft flex flex-col justify-between">
                      <div>
                        ...
                        <div className="space-y-2 mt-4 text-xs font-bold text-slate-500 uppercase">
                          {[
                            {
                              l: "RTO Packaging Loss",
                              v: accountStatus.rto_packaging_loss,
                              c: "text-slate-800",
                            },
                            {
                              l: "Unreturned Packages Loss",
                              v: accountStatus.return_not_received_loss,
                              c: "text-red-500",
                            },
                            {
                              l: "Mismatch & Wrong Return Loss",
                              v: accountStatus.wrong_damage_missing_returns,
                              c: "text-red-500",
                            },
                            {
                              l: "Payment Leakage Loss",
                              v: accountStatus.payment_loss,
                              c: "text-red-500",
                            },
                          ].map((row, idx) => (
                            <div
                              key={idx}
                              className="flex items-center justify-between pb-1 border-b border-slate-50"
                            >
                              <span className="text-[10px] font-black text-slate-400">
                                {row.l}
                              </span>
                              <span className={`font-black ${row.c}`}>
                                ₹{formatCount(row.v)}
                              </span>
                            </div>
                          ))}
                      ...
                      <div className="mt-4 p-3 rounded-xl bg-green-50/50 border border-green-100 flex items-center justify-between text-xs">
                        ...
                        <h4 className="font-black text-green-800 text-base select-all">
                          ₹{formatCount(accountStatus.net_profit)}
                        </h4>
                      </div>
                    </div>
                  </div>
                          ...
                          <div className="flex flex-col gap-1.5 min-w-[200px] flex-1">
                            ...
                            <div className="relative group">
                              ...
                              <input
                                ...
                                onChange={(e) =>
                                  setPaymentsSkuDraft(e.target.value)
                                }
                                placeholder="Search SKU..."
                                className="h-9 w-full bg-slate-50 border border-slate-200 rounded-xl pl-10 pr-4 text-xs font-bold text-slate-700 outline-none focus:bg-white focus:border-primary transition-all shadow-inner"
                              />
                          ...
                          <div className="flex flex-col gap-1.5 min-w-[200px] flex-1">
                            ...
                            <div className="relative group">
                              ...
                              <input
                                ...
                                onChange={(e) =>
                                  setPaymentsOrderIdDraft(e.target.value)
                                }
                                placeholder="Search Order ID..."
                                className="h-9 w-full bg-slate-50 border border-slate-200 rounded-xl pl-10 pr-4 text-xs font-bold text-slate-700 outline-none focus:bg-white focus:border-primary transition-all shadow-inner"
                              />
                          ...
                          <div className="flex flex-col gap-1.5 w-44">
                            ...
                            <div className="relative">
                              <select
                                ...
                                onChange={(e) =>
                                  setPaymentsOrderFilterDraft(e.target.value)
                                }
                                className="h-9 w-full appearance-none rounded-xl border border-slate-200 bg-slate-50 px-4 text-xs font-bold text-slate-700 outline-none focus:bg-white focus:border-primary transition-all shadow-inner hover:border-slate-300"
                              >
                                <option value="All">All Orders</option>
                      ...
                      {paymentsTotal > 0 && (
                        <div className="flex items-center justify-between mt-4 pt-3 border-t border-slate-100 text-xs text-slate-400 font-bold">
                          <div>
                            Showing{" "}
                            {Math.min(
                              (paymentsPage - 1) * paymentsLimit + 1,
                              paymentsTotal,
                            )}{" "}
                            -{" "}
                            {Math.min(
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <span className="font-extrabold text-slate-700">
                              ...
                              {Math.ceil(paymentsTotal / paymentsLimit) || 1}
                            </span>
                            <button
                              disabled={
                                paymentsPage >=
                                Math.ceil(paymentsTotal / paymentsLimit)
                              }
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
                    {[
                      ...
                      <div
                        key={i}
                        className="bg-white rounded-2xl border border-slate-100 p-4 shadow-soft hover:shadow-card hover:-translate-y-0.5 transition-all flex items-start justify-between"
                      >
                        <div className="space-y-3">
                          <div
                            className={`p-2.5 rounded-xl ${card.color.bg} ${card.color.text} w-fit`}
                  ...
                  <div className="bg-white rounded-2xl border border-slate-100 p-5 shadow-soft">
                    <div className="flex items-center justify-between mb-4">
                      <h3 className="text-sm font-black text-slate-800 uppercase tracking-wider border-l-2 border-primary pl-2">
                      ...
                      <table className="w-full text-left border-collapse">
                        ...
                        <tbody>
                          <tr>
                            <td
                              colSpan={4}
                              className="py-12 text-center text-xs font-bold text-slate-400 italic"
                            >
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 items-stretch">
                    ...
                    <div className="bg-white rounded-2xl border border-slate-100 p-5 shadow-soft flex flex-col justify-between">
                      <div>
                        ...
                        <div className="space-y-2 mt-4">
                          {[
                            ...
                            {
                              filterKey: "not-received",
                              key: "return_not_received",
                              title: "Pending/Unreturned",
                              color: "text-rose-500",
                              bg: "bg-rose-50",
                            },
                            {
                            ...
                            return (
                              <div
                                key={stage.key}
                                onClick={() =>
                                  setReturnsFilter(stage.filterKey)
                                }
                                className={`flex items-center justify-between p-3 rounded-xl border transition-all cursor-pointer ${
                                  returnsFilter === stage.filterKey
                                    ? "bg-primary/5 border-primary shadow-sm"
                                    : "bg-slate-50/30 border-slate-100 hover:bg-slate-50"
                                }`}
                              >
                                <div className="flex items-center gap-2">
                                  <div
                                    className={`h-2.5 w-2.5 rounded-full ${stage.color.replace("text", "bg")}`}
                                  />
                      ...
                      <div className="mt-4 pt-3 border-t border-slate-100 flex items-center justify-between text-xs">
                        <span className="font-extrabold text-slate-500">
                          Average Return Logistics Cost
                        </span>
                        <span className="font-black text-red-600">
                          ₹
                          {formatCount(
                            dashboardData?.averages?.avg_return_charge,
                          )}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="bg-white rounded-2xl border border-slate-100 p-5 shadow-soft flex flex-col lg:col-span-2">
                      <div className="flex items-center gap-2 mb-4">
                        ...
                        <div>
                          <h3 className="text-sm font-black text-slate-800 uppercase tracking-wider">
                            Returned Orders stage Tracking
                          </h3>
                          <p className="text-[10px] font-bold text-slate-400 mt-0.5">
                            Distribution graph of courier return stages
                          </p>
                        </div>
                      </div>
                  ...
                  >
                    ...
                    <div className="bg-white rounded-2xl border border-slate-100 p-5 shadow-soft min-w-0">
                      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3 mb-5 border-b border-slate-50 pb-4">
                        <div className="flex items-center gap-2">
                          <h3 className="text-lg font-black text-slate-800 uppercase tracking-tight">
                            {returnsFilter === "transit" &&
                              "Return In Transit Records"}
                            {returnsFilter === "received" &&
                              "Received Returns Records"}
                            {returnsFilter === "not-received" &&
                              "Pending/Unreturned Records"}
                            {returnsFilter === "mismatch" &&
                              "Damaged / Wrong Returns Records"}
                          </h3>
                          <span className="bg-amber-50 text-amber-600 text-xs font-extrabold px-2.5 py-1 rounded-full">
                            {formatCount(returnsTotal)}
                          </span>
                        </div>
                        <div className="flex flex-wrap items-center gap-2.5 lg:justify-end">
                          <select
                          ...
                          <div className="flex flex-col gap-1.5 min-w-[200px] flex-1">
                            ...
                            <div className="relative group">
                              ...
                              <input
                                ...
                                onChange={(e) =>
                                  setReturnsSkuDraft(e.target.value)
                                }
                                placeholder="Search SKU..."
                                className="h-9 w-full bg-slate-50 border border-slate-200 rounded-xl pl-10 pr-4 text-xs font-bold text-slate-700 outline-none focus:bg-white focus:border-primary transition-all shadow-inner"
                              />
                          ...
                          <div className="flex flex-col gap-1.5 min-w-[200px] flex-1">
                            ...
                            <div className="relative group">
                              ...
                              <input
                                ...
                                onChange={(e) =>
                                  setReturnsOrderIdDraft(e.target.value)
                                }
                                placeholder="Search Order ID..."
                                className="h-9 w-full bg-slate-50 border border-slate-200 rounded-xl pl-10 pr-4 text-xs font-bold text-slate-700 outline-none focus:bg-white focus:border-primary transition-all shadow-inner"
                              />
                      ...
                      {returnsTotal > 0 && (
                        <div className="flex items-center justify-between mt-4 pt-3 border-t border-slate-100 text-xs text-slate-400 font-bold">
                          <div>
                            ...
                            -{" "}
                            {Math.min(returnsPage * returnsLimit, returnsTotal)}{" "}
                            of {returnsTotal}
                          </div>
                          <div className="flex items-center gap-2">
                            <button
                              ...
                              onClick={() =>
                                setReturnsPage((p) => Math.max(p - 1, 1))
                              }
                              className="p-1.5 rounded-lg border border-slate-200 hover:bg-slate-50 disabled:opacity-40 disabled:hover:bg-transparent transition-colors cursor-pointer"
                            >
                              <FiChevronLeft size={16} />
                            ...
                            <span className="font-extrabold text-slate-700">
                              Page {returnsPage} of{" "}
                              {Math.ceil(returnsTotal / returnsLimit) || 1}
                            </span>
                            <button
                              disabled={
                                returnsPage >=
                                Math.ceil(returnsTotal / returnsLimit)
                              }
